...
(function () {
	...
	var Cleaver = function Cleaver(opts) {
		var THIS = this,
		...
		blade = opts.blade,
		    keyedRange = function keyedRange(value, object) {
			var i, range, min, max;
			for (i in object) {
				range = i.split(/\s*[-+]\s*/gm).filter(function (a) {
					return a;
				});
				min = +range[0];
				max = +range[1];
				if (value >= min && (!max || value < max)) return object[i];
			}
		},
