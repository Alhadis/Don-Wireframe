...
(function () {
	...
	var RangeSlider = function RangeSlider(node, opts) {
		var THIS = this,
		    opts = opts || {},
		...
		items = node.children,
		    ...
		    /** Arrowhead pointing to selected product */
		details = opts.details,
		    /** Item details, displayed on selection */
		prev = opts.prev,
   ...
   */
		refresh = function refresh() {
			...
			if (pointer) {
				...
				pointer.style[WIN.CSS_TRANSFORM] = translateX.join((activeIndex - columnOffset) / THIS.columns * width - width / 2 + THIS.columnWidth / 2 + "px");
			}
		};
		...
		WIN.addEventListener("resize", function () {
			...
			if (activeIndex >= col) THIS.activeIndex = col - 1;
		...
		items.forEach(function (e, i) {
			e.addEventListener("mouseover", function () {
				if (i >= columnOffset && i <= THIS.columns + columnOffset) THIS.activeIndex = i;
			});
		});

		/** Navigation handlers */
		prev && prev.addEventListener(CLICK, function (e) {
			--THIS.columnOffset;--THIS.activeIndex;
		});
