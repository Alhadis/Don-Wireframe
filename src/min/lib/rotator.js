...
(function () {
	...
	function pruneTextNodes(e, i) {
		if (!e || !e.childNodes.length) return e;e.normalize();var t = e.lastChild,
		    n = /^\s*$/;if (3 === t.nodeType && (!i || n.test(t.data))) {
			e.removeChild(t);t = e.lastChild;if (!t) return e;
		}while (t = t.previousSibling) if (3 === t.nodeType && (!i || n.test(t.data))) e.removeChild((t = t.nextSibling).previousSibling);return e;
	};
	/*>/
 ...
 /*<*/
	...
	Rotator = function Rotator(el, args) {
		...
		OBJ.defineProperties(THIS, {
			...
			currentSlide: {
				...
				set: function set(i) {
					...
					if (i < 0) i = THIS.wrap ? max : 0;else if (i > max) i = THIS.wrap ? 0 : max;
			...
			autoplay: {
				...
				set: function set(i) {
					...
					clearTimeout(autoplayTimer);
					if (i) autoplayTimer = setTimeout(function () {
						++THIS.currentSlide;
					}, i);
		...
		if (!THIS.slides.children) OBJ.defineProperty(THIS.slides, "children", {
			get: function get() {
				for (var output = [], i = 0, c = this.childNodes, l = c.length; i < l; ++i) if (Node.ELEMENT_NODE === c[i].nodeType) output.push(c[i]);
				return output;
			}
		});

		/** Make DOM traversal easier. */
		if (!THIS.keepTextNodes) pruneTextNodes(THIS.slides);

		/** Event listeners */
		!THIS.navPrev || THIS.navPrev[ADD_LISTENER](POINT_EVENT, function (e) {
			--THIS.currentSlide;
		});
		!THIS.navNext || THIS.navNext[ADD_LISTENER](POINT_EVENT, function (e) {
			++THIS.currentSlide;
		});
		...
		DOC[ADD_LISTENER]("visibilitychange", function () {
			...
			if (THIS.autoplay) {
				...
				if (state === "unloaded" || state === "hidden") clearTimeout(autoplayTimer);
